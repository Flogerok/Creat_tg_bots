...
# функция поиска по бд по остановке
def g_poisk_roat_lst():
    with sql.connect("Raspisanie_bus_Test1BD.db3") as db:
        curs = db.cursor()
        curs.execute("SELECT stop_bus FROM stop_roat WHERE roat_number=?", ([485.]))

        return curs.fetchall()
print("==== поиск по бд====", type(g_poisk_roat_lst()), g_poisk_roat_lst())
# финкция строки поиска бд
def g_poisk_roat_str():
    poisk_roat_lst = ""
    for i in g_poisk_roat_lst():
        poisk_roat_lst += f'{i} \n'
    return poisk_roat_lst
print("0 = poisk_roat_str", type(g_poisk_roat_str()),"\n",  g_poisk_roat_str())
...
def callback_query(call):
    ...
    # Создаем меню выбора маршруток
        ...
        roat = call.data
        marcup = InlineKeyboardMarkup()
        marcup.add(InlineKeyboardButton('ДА .Погнали!', callback_data="/stop_roat"),
                   InlineKeyboardButton("СТОП! Назат!", callback_data="/add_start"))
        bot.send_message(
            call.message.chat.id,
